...
MODULE:=github.com/ensure-stack/coordination-operator
...
KIND_CLUSTER_NAME:="coordination-operator" # name of the kind cluster for local development.

# Container
IMAGE_ORG?=quay.io/app-sre
COORDINATION_OPERATOR_MANAGER_IMAGE?=$(IMAGE_ORG)/coordination-operator-manager:$(VERSION)
...
all: \
	bin/linux_amd64/coordination-operator-manager
...
generate: $(CONTROLLER_GEN)
	...
	@controller-gen crd:crdVersions=v1,generateEmbeddedObjectMeta=true \
		rbac:roleName=coordination-operator-manager \
		paths="./..." \
...
test-e2e-local: | dev-setup load-coordination-operator test-e2e
...
delete-kind-cluster: $(KIND)
	@echo "deleting kind cluster coordination-operator-e2e..."
	@(source hack/determine-container-runtime.sh; \
...
load-coordination-operator: build-image-coordination-operator-manager
	@source hack/determine-container-runtime.sh; \
		$$KIND_COMMAND load image-archive \
			.cache/image/coordination-operator-manager.tar \
			--name=$(KIND_CLUSTER_NAME);
.PHONY: load-coordination-operator

# Template deployment
config/deploy/deployment.yaml: FORCE $(YQ)
	@yq eval '.spec.template.spec.containers[0].image = "$(COORDINATION_OPERATOR_MANAGER_IMAGE)"' \
...
setup-coordination-operator: $(YQ) load-coordination-operator config/deploy/deployment.yaml
	@echo "installing Addon Operator $(VERSION)..."
	@(source hack/determine-container-runtime.sh; \
		kubectl apply -f config/deploy; \
		echo -e "\nwaiting for deployment/coordination-operator..."; \
		kubectl wait --for=condition=available deployment/coordination-operator -n coordination-operator --timeout=240s; \
		echo; \
	) 2>&1 | sed 's/^/  /'
.PHONY: setup-coordination-operator

## Installs Addon Operator CRDs in to the currently selected cluster.
setup-coordination-operator-crds: generate
	@for crd in $(wildcard config/deploy/*.openshift.io_*.yaml); do \
		kubectl apply -f $$crd; \
	done
.PHONY: setup-coordination-operator-crds
...
## Build all images.
build-images: \
	build-image-coordination-operator-manager
.PHONY: build-images

## Build and push all images.
push-images: \
	push-image-coordination-operator-manager
.PHONY: push-images
